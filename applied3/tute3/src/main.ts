...
function mousePosObservable() {
    ...
    source$
        .pipe(map(({clientX, clientY}) => ({x: clientX, p: clientX + ", " + clientY}))) // This must be pure
        .subscribe(({x, p}) => {x > 400 ? elem.classList.add("highlight") : elem.classList.remove("highlight");
                                    elem.textContent = p;}); // Side effects should be contained here
}
...
function animatedRect() {
    ...
    const source$ = interval(1) //// produces a stream of increasing numbers, emitted every “period” milliseconds

    const move$ = source$
        .pipe(
            takeUntil(source$.pipe(filter(x => x === 100))),

            scan((acc, _)=> acc + 2, startProps.x),
        )
...
function animatedRect2() {
    ...
    const moveDownRight$ = interval(10)
        .pipe(
            // Stop taking values after some amount of time (1.41s = 1410 ms)
            takeUntil(timer(1410)),
            // Update position of rectangle
            scan((acc: {x: number, y: number}, _) => ({x: acc.x + 1, y: acc.y + 1}), {x: startProps.x, y: startProps.y}),
        )
...
function keyboardControl() {
     ...
     */
    const fromKey = (keyCode: string, coordinates: {x: number, y: number}) =>
        key$.pipe(
    ...
    //     .pipe(
    //         scan((acc: {x: number, y: number}, value: {coordinate: "x"|"y", amount: number}) => 
    //             (value.coordinate === "x" ? {x: acc.x + value.amount, y: acc.y} : {x: acc.x, y: acc.y + value.amount}), 
    ...
    merge(left$, down$, up$, right$)
        .pipe(
            scan((acc: {x: number, y: number}, coordinates: {x: number, y: number}) => 
                ({x: acc.x + coordinates.x, 
                  y: acc.y + coordinates.y}), 
                {x: startProps.x, y: startProps.y}))
...
function printWithDelay() {
    ...
    // csvText$.pipe(  mergeMap((csvtext: string): string[] => csvtext.split("\n")),
    //                 map((text) : {delay: number, text: string} => {const splitText = (text as string).split(","); return {delay: parseInt(splitText[0]), text: splitText[1]}}),
    //                 mergeMap(x => interval(1000).pipe(map(i => i === x.delay ? x.text : i)))
    //                 ).//subscribe(x => console.log(x))
    //                 subscribe(x => {if (typeof x === "string") console.log(x)}); 

    csvText$.pipe(  mergeMap((csvtext: string): string[] => csvtext.split("\n")),
                    map((text) : {delay: number, text: string} => {const splitText = (text as string).split(","); return {delay: parseInt(splitText[0]), text: splitText[1]}}),
                    mergeMap(x => timer(x.delay*1000).pipe(map(i => x.text)))
                    ).//subscribe(x => console.log(x))
                    subscribe(x => {if (typeof x === "string") console.log(x)}); 
